# these functions are adapt from initial model from https://github.com/GoogleCloudPlatform/ipython-soccer-predictions
# changes include:
#  + change code for running in Python3 instead of Python 2.7

"""
    Predicts football outcomes using logistic regression.
"""
...
def _drop_unbalanced_matches(data):
    ...
    while index < len(data) - 1:
        ...
        if skipped:
            pass
        elif data.iloc[index]['match_api_id'] == data.iloc[index+1]['match_api_id']:
            keep.append(True)
            keep.append(True)
            index += 2
        else:
...
def _splice(data):
    ...
    data = data.copy()
    opp = data.copy()
    opp_cols = ['away_%s' % (col,) for col in opp.columns]
...
def extract_predictions(data, predictions):
    ...
    for game in range(len(data)/2):
        if data['match_api_id'].iloc[game*2] != data['match_api_id'].iloc[game*2+1]:
            raise Exception('Unexpeted match id %d vs %d', (
                               data['match_api_id'].iloc[game * 2],
                               data['match_api_id'].iloc[game * 2 + 1]))
...
def _check_data(data):
    ...
    matches = data['match_api_id']
    teams = data['home_team_api_id']
    op_teams = data['away_team_api_id']
    while i < len(data) - 1:
